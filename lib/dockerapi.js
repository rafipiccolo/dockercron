...
let execFilePromise = require('./execFilePromise');
...
let safeGetTunnel = queue(getTunnel);
...
async function getTunnel(host) {
    ...
    let localsocket = `/tmp/proxy.${host}.sock`;
...
async function dockerApi(host, method, path, postdata, timeout, onLine, needdemux) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (timeout)
            x = setTimeout(() => {
                let err = new Error(`timeout when ${path}`);
                err.code = 'TIMEOUT';
                reject(err);
            }, timeout * 1000);
...
async function createTunnel(localsocket, host, remotesocket) {
    ...
    let tunnel = spawn('ssh', ['-L', `${localsocket}:${remotesocket}`, '-qC', host]);
...
async function info(params) {
    let { statusCode, body } = await dockerApi(params.host, 'GET', '/v1.41/info', null, params.timeout);
...
async function listTasks(params) {
    ...
    let { statusCode, body } = await dockerApi(params.host, 'GET', url, null, params.timeout);
...
async function getTask(params) {
    let { statusCode, body } = await dockerApi(params.host, 'GET', `/v1.41/tasks/${params.id}`, null, params.timeout);
...
async function listNodes(params) {
    let { statusCode, body } = await dockerApi(params.host, 'GET', '/v1.41/nodes', null, params.timeout);
...
async function getNode(params) {
    let { statusCode, body } = await dockerApi(params.host, 'GET', `/v1.41/nodes/${params.id}`, null, params.timeout);
...
async function listServices(params) {
    let { statusCode, body } = await dockerApi(params.host, 'GET', '/v1.41/services', null, params.timeout);
...
async function listContainers(params) {
    let { statusCode, body } = await dockerApi(params.host, 'GET', '/v1.41/containers/json', null, params.timeout);
...
async function getService(params) {
    let { statusCode, body } = await dockerApi(params.host, 'GET', `/v1.41/services/${params.id}`, null, params.timeout);
...
async function getContainer(params) {
    let { statusCode, body } = await dockerApi(params.host, 'GET', `/v1.41/containers/${params.id}/json`, null, params.timeout);
...
async function getExec(params) {
    let { statusCode, body } = await dockerApi(params.host, 'GET', `/v1.41/exec/${params.id}/json`, null, params.timeout);
...
async function getEvents(params) {
    return await dockerApi(params.host, 'GET', `/v1.41/events`, null, params.timeout, (line) => {
...
async function exec(params) {
    let res1 = await dockerApi(params.host, 'POST', `/v1.41/containers/${params.id}/exec`, params.options, params.timeout);
    ...
    let res2 = await dockerApi(
        params.host,
        'POST',
        `/v1.41/exec/${execId}/start`,
        {},
        params.timeout,
        (line) => {
