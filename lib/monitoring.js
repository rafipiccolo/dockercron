...
function errorToObject(argument) {
    ...
    if (Array.isArray(argument.stack))
        obj.stack = argument.stack;
    else
        obj.stack = (argument.stack || '').split('\n');
...
function log(level, key, message, obj) {
    ...
    if (json.err) json.err = errorToObject(json.err);
    console.log(`${level == 'error' ? red : blue}${key}${reset} ${message} ${obj ? JSON.stringify(obj, null, 4) : ''}`);
