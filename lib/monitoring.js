...
function banmiddleware(app) {
    ...
    app.get('/ban', async (req, res, next) => {
        let fileName = path.resolve(`${__dirname}/../views/ban.html.twig`);
        res.type('html');
    ...
    return function (req, res, next) {
        ...
        if (req.originalUrl.match(/(\$|\%24)(\{|\%7b).*j.*n.*d.*i.*(\:|\%3a)/i))
            return req.ban("Log4Shell");
        if (req.originalUrl.match(/%3C(%20)*script(%20)*%3E/i))
            return req.ban("balise script dans l'url");

        // for (let i in req.body) {
        //     if (typeof req.body[i] == 'string' && req.body[i].match(/<\s*script\s*>/i)) {
