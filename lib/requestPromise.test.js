...
const uniqid = Math.random() * 10_000;
const jar = createJar();

describe('requestPromise.js', () => {
    ...
    // curl -X POST -F 'image=@./lib/samples/httpbin.jpeg' -F test=test 'https://httptest.raphaelpiccolo.com/'
    it('should post file using formdata', async () => {
        const { response, body } = await requestPromise({
            method: 'POST',
            url: 'https://httptest.raphaelpiccolo.com/',
            formData: { test: 'test', file: fs.createReadStream(`${__dirname}/samples/httpbin.jpeg`) },
            json: true,
        });
        assert.equal(response.statusCode, 200);
        assert.deepEqual(body.body, { test: 'test' });
        assert.equal(body.files.length, 1);
        assert.equal(body.files[0].fieldname, 'file');
        assert.equal(body.files[0].mimetype, 'image/jpg');
        assert.equal(body.files[0].originalname, 'httpbin.jpeg');
        assert.equal(body.files[0].size, 35_588);
    });
